  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto space-y-5 px-3 sm:px-6 py-4 sm:py-6 overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* ── VERSION MOBILE : Solde Total (en haut) + Salaire Net + Bouton (sm:hidden) ── */}
        <div className="flex sm:hidden flex-col gap-2.5 w-full">
          {/* Solde Total (uniquement sur mobile, juste en haut de salaire net) */}
          <div className="bg-[#18181B] border border-[#EF4444]/40 bg-gradient-to-r from-[#EF4444]/15 via-[#18181B] to-[#18181B] rounded-xl px-3.5 py-2.5 flex items-center justify-between shadow-md shadow-black/40">
            ...
            <p className="text-base font-black text-[#FAFAFA] tabular-nums">
              {formatCurrencyValue(totalBalance, displayCurrency as any, locale, true)}
            </p>
          </div>

          {/* Salaire Net Mensuel (juste en-dessous sur mobile) */}
          <div className="bg-[#18181B] border border-[#27272A] rounded-xl px-3.5 py-2 flex items-center justify-between">
            <p className="text-[10px] text-[#A1A1AA] font-bold uppercase tracking-wider">
              {t.dashboard.paydayCard.salaryLabel}
            </p>
            <p className="text-base font-extrabold text-[#FAFAFA] tabular-nums">
              {formatCurrencyValue(netSalary, displayCurrency as any, locale, true)}
            </p>
          </div>

          {/* Bouton Ajouter une dépense sur mobile */}
          <button
            onClick={() => onOpenModal ? onOpenModal() : setShowExpenseModal(true)}
            className="btn-primary py-2.5 px-4 text-xs w-full justify-center flex items-center gap-2 shadow-lg shadow-[#EF4444]/25 cursor-pointer"
          >
        ...
        {/* ── VERSION ORDINATEUR / TABLETTE (identique à l'original) ── */}
        <div className="hidden sm:flex items-center gap-3 sm:gap-3.5 self-auto justify-end">
          <div className="text-right bg-[#18181B] border border-[#27272A] rounded-xl px-3.5 py-2">
            <p className="text-[10px] text-[#A1A1AA] font-bold uppercase tracking-wider">
              {t.dashboard.paydayCard.salaryLabel}
            </p>
          ...
          <button
            onClick={() => onOpenModal ? onOpenModal() : setShowExpenseModal(true)}
            className="btn-primary py-2.5 px-4 text-xs shrink-0 cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            <span>{t.dashboard.addTransaction}</span>
          </button>
